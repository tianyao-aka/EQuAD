...
global_b = '0.3'
# for global_b in ['0.599','0.81','0.91']:
for global_b in ['0.40']:
    num_train_perc = 3000
    num_val_perc = 1000
    num_test_perc = 1000
    ...
    for _ in tqdm(range(num_train_perc)):
        ...
        if base_num == 1:
            ...
            high_corr_list.append(1)
        if base_num == 2:
            base = 'ladder'
            width_basis = np.random.choice(range(8, 12))
            high_corr_list.append(0)
        if base_num == 3:
            base = 'wheel'
    ...
    np.save(f'../data/mSPMotif-{global_b}/raw/train.npy',
            (edge_index_list, label_list, ground_truth_list, role_id_list, pos_list,high_corr_list))
