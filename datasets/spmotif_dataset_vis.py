...
class SPMotif(InMemoryDataset):
    ...
    def process(self, get_data_list=False):
        ...
        if 'tspmotif' in self.root.lower() or 'dspmotif' in self.root.lower():
            if idx==0:
                edge_index_list, label_list, base_list, ground_truth_list, role_id_list, pos, motifs,high_corr = np.load(osp.join(
                ...
                high_corr = np.zeros(len(label_list),dtype=np.int)-1  
        elif '0.6' in self.root or '0.7' in self.root:
            if idx==0:
                edge_index_list, label_list, base_list, ground_truth_list, pos,high_corr = np.load(osp.join(
                self.raw_dir, self.raw_file_names[idx]),allow_pickle=True)
            else:
                edge_index_list, label_list, base_list, ground_truth_list, pos = np.load(osp.join(
                self.raw_dir, self.raw_file_names[idx]),allow_pickle=True)
                high_corr =   np.zeros(label_list.shape,dtype=np.int)-1  
            role_id_list = np.zeros(label_list.shape,dtype=np.int)-1
            motifs = np.zeros(label_list.shape,dtype=np.int)-1
        else:
            if idx==0:
                edge_index_list, label_list, ground_truth_list, role_id_list, pos,high_corr = np.load(osp.join(
                    self.raw_dir, self.raw_file_names[idx]),allow_pickle=True)
            else:
        ...
        data_list = []
        for idx, (edge_index, y, group,ground_truth, z, p, mid,hc) in \
        enumerate(zip(edge_index_list, label_list, base_list, ground_truth_list, role_id_list, pos, motifs,high_corr)):
            edge_index = torch.from_numpy(edge_index)
            high_corr_tensor = torch.tensor([hc]).long()
