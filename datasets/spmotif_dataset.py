...
class SPMotif(InMemoryDataset):
    ...
    def process(self, get_data_list=False):
        ...
        if 'tspmotif' in self.root.lower() or 'dspmotif' in self.root.lower():
        ...
        elif '0.6' in self.root or '0.6' in self.root:
            ...
            edge_index_list, label_list, ground_truth_list, role_id_list, pos,env_list = np.load(osp.join(
                self.raw_dir, self.raw_file_names[idx]),allow_pickle=True)
            motifs = np.zeros(label_list.shape,dtype=np.int)-1
            base_list = np.zeros(label_list.shape,dtype=np.int)-1
        
        data_list = []
        for idx, (edge_index, y, group,ground_truth, z, p, mid,env) in \
        enumerate(zip(edge_index_list, label_list, base_list, ground_truth_list, role_id_list, pos, motifs,env_list)):
            ...
            env_id = torch.tensor(env-1,dtype=torch.long).view(-1,)
